...
function WrapDecorator(props: React.PropsWithChildren<DecoratorProps>) {
  ...
  return (
    <div
      tabIndex={1}
      className={`${isActive ? "border border-blue-500 border-solid" : ""} 
           ${
             widgetCategory == WidgetCategory.Container
               ? "p-4 bg-blue-200 "
               : ""
           }
  ...
  switch (widgetCategory) {
    ...
    case WidgetCategory.Decorator:
      return (
        <DragWrap
          itemType={ItemType.actor}
          item={{ componentId: id, itemType: ItemType.actor }}
        >
          {children}
        </DragWrap>
      );
...
export function FormItemWrap(
  props: React.PropsWithChildren<FormItemWrapProps>
) {
